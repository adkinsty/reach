...
aim <- seq(0,1,.001) # approx -48 to 48 by 1 px
G <- length(grid)
pg <- array(rep(0,G*S),dim=c(G,S))
pl <- array(rep(0,G*S),dim=c(G,S))
pgl <- array(rep(0,G*S),dim=c(G,S))
pn <- array(rep(0,G*S),dim=c(G,S))
for (j in 1:J) {
  print(j)
  tmp <- train %>% filter(sub_id == j & block > 5)
  sigma <- array(c(var(tmp$x),cov(tmp$x,tmp$y),cov(tmp$x,tmp$y),var(tmp$y)),dim=c(2,2))
  
  sim_dist <- function(x,s=sigma,n=1e3) {
...
input <- list(N=N,
              ...
              jj=jj,
              aim=aim,
              pg=pg,pl=pl,pgl=pgl,pn=pn,
              y=y,
              get_yrep=1,
              get_log_lik=1,
              prior_only=0)

model <- stan_model(file="modeling/stan/hier_uni_norm_nonlin_EU.stan",
                    model_name="hier_uni_norm_nonlin_EU",
...
write_rds(mcmc_est,path="modeling/stan/rds/H.rds")
