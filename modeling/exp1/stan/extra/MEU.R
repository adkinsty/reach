...
input <- list(N=N,J=J,C=C,G=G,
              cc=cc,cond=cond,jj=jj,
              aim=aim,pg=pg,pl=pl,pgl=pgl,
              y=y,
              get_yrep=1,
              get_log_lik=1,
              prior_only=0)
...
model <- stan_model(file="modeling/stan/MEU.stan",
                    ...
                    includes = paste0(
                      '\n#include "',
                      file.path(getwd(),'modeling/stan/hpp/get_max_idx.hpp'),'"\n')
                    )
map_est = optimizing(
  object=model,
  data=input,
  ...
  algorithm="Newton")
map_est$par$alpha
mcmc_est <- readRDS("modeling/stan/rds/MEU.rds")
mcmc_est <- sampling(
  object=model,
  data=input,
  chains=2,
  ...
  verbose=TRUE,
  algorithm="NUTS")
write_rds(mcmc_est,path="modeling/stan/rds/MEU_prior.rds")


print(mcmc_est,pars = c("sigma_m","sigma_s"),probs = c(.025,.975))
stan_plot(mcmc_est,pars = c("sigma_m","sigma_s","sigma"))
stan_plot(mcmc_est,pars = c("beta"))
...
pred %>% 
  ...
  stat_bin_hex(aes(x=mu_rep,y=mu_obs),binwidth = .05,alpha=.9) +
  scale_fill_viridis_b(option="A") +
