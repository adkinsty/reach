# Title     : Uses MLE to obtain predictions for MSWU-gamma model (restricted gamma)
...
get_w <- function(p,gamma) {
  # non-linear probability transform
...
get_mu <- function(p,k,alpha,beta,gamma){
  hyp <- seq(0,1,.01) # aims
...
for (j in 1:J) {
  ...
  objective <- function(par,P=all_p,y=obs,kk=cond_id,s=sig) {
    ...
    for (k in 1:K) {
      p <- get_p(k, P)
      mk <- append(mk,get_mu(p,k,alpha,beta,gamma))
    }
    mu <- c()
    for (n in 1:length(y)) {
      mu[n] = mk[kk[n]]
  ...
  for (k in 1:10) {
    p <- get_p(k, all_p)
    mu_cond[k,j] <- get_mu(p,k,param[1,j],param[2,j],param[3,j])
  }
}

write_rds(param,"data/clean/exp1/MSWU_param.rds")
write_rds(mu_cond,"data/clean/exp1/MSWU_aims.rds")
