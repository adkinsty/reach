...
# model aimpoints
meg_aim <- as.matrix(read.csv("data/clean/exp1/MEG_aims.csv",row.names = 1))
mleg_aim <- as.matrix(read.csv("data/clean/exp1/MLEG_aims.csv",row.names = 1))
mswu_aim <- as.matrix(read.csv("data/clean/exp1/MSWU_aims.csv",row.names = 1))
h_aim <- as.matrix(read.csv("data/clean/exp1/h_aims.csv",row.names = 1))
aims_array <- array(dim=c(10,22,4))
aims_array[,,1] <- mswu_aim
aims_array[,,2] <- h_aim
...
obs_pred <- data %>% group_by(sub_id,cond_id) %>%
  ...
  mutate(pred=x*8.55,
         ...
         model=ifelse(model==1,"MSWU",
                      ifelse(model==2,"H",
                              ifelse(model==3,"MEV","MELV"))),
         model=factor(model,levels=c("MSWU","H","MEV","MELV"))) %>% ungroup()
...
obs_pred %>%
  ...
  scale_colour_manual("Model",
                      values=c("#4ca5ff","#fe8a2f","#66cc99","#b97c9b"),
                      labels=c("MSWU","H","MEV","MELV")) +
  theme_tufte(base_size = 10, base_family = "sans") +
