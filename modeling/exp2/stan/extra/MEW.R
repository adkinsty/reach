...
for (j in 1:J) {
  ...
  for (c in 1:C) {
    EW = pg*1 + pgl*(1+sign(cond[c])) + pl*sign(cond[c]) + pn*0
    aim[c,j] = hyp[which.max(EW)]
  }
}
...
mcmc_est <- sampling(
  ...
  warmup=2500,
  cores=4)
write_rds(mcmc_est,path="modeling/stan/rds/MEW.rds")
