...
input <- list(
  ...
  C=3,                                               # num unique conditions
  cond=sort(unique(data$ratio)),                     # unique conditions
  cc=data$cond_id,                                   # condition idx for trial
...
model <- stan_model(
  file="modeling/stan/nonlinear_regression.stan",
  model_name="nonlinear_regression"
)
...
print(fit2,pars = c("sigma_m","sigma_s","sigma"),probs = c(.025,.05,.10,.90,.95,.975))
stan_plot(fit,pars = c("sigma_m","sigma_s","sigma"))

print(fit,pars = c("gamma","beta"),probs = c(.025,.05,.10,.90,.95,.975))
stan_plot(fit,pars = c("gamma","beta"))
...
pred %>% 
  ...
  geom_point(aes(x=mu_rep,y=mu_obs,colour=ratio),size=.1,alpha=.04) +
  scale_color_viridis_c(option = "B",begin=.1,end=.9) +
  theme_tufte(base_family = "sans",base_size=15) +
  theme(axis.line = element_line(size=.25),
