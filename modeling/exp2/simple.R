...
m <- brm(bf(zx ~ 1 + rat + zla + zsd + (1 |sub_id), 
            sigma ~ 1 + (1 | sub_id)),
         data=input,
...
mo_loo <- loo(mo, save_psis = TRUE, cores = 4)
mv_loo <- loo(mv, save_psis = TRUE, cores = 4)
...
ggsave("visuals/raw/exp2/exp2_noise_reduction.pdf",units="in",height=2,width=2)  
