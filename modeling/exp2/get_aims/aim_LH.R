# Title     : Get aimpoints for LH (linear heuristic) model
# Objective : inputs to stan model
# Created by: adkinsty
# Updated on: 2/5/21

setwd("/Users/adkinsty/Box/LeeLab/Experiments/Exp_files/reach/")

library(tidyverse)
library(optimx)

get_mu <- function(loss,intercept,slope){
  gain <- 1
  ratio <- abs(loss / gain)
  v <- ifelse(ratio == 0, -1, ifelse(ratio == 1, 0, 1))
  mu <- intercept + slope * v
  return(mu)
}
get_obs <- function(data,j) {
  tmp = data %>% filter(sub_id == j)
  return(tmp$x)
}
get_loglik <- function(x,mu,sigma) {
  return(sum(-dnorm(x,mu,sigma,log=TRUE)))
}

train <- read_csv("data/clean/exp2/train_data.csv") %>%
  filter(x > -2 & x < 2 & y > -2 & y < 2)

data <- read_csv("data/clean/exp2/test_data.csv") %>%
  filter(x > -2 & x < 2 & y > -2 & y < 2)

# key variables
jj <- data$sub_id
kk <- data$cond_id
J <- length(unique(jj))
K <- length(unique(kk))

param <- matrix(nrow=2, ncol=J) #  MLE parameters
mu_cond <- matrix(nrow=3,ncol=J) # MLE aim-points

# Maximum Likelihood Estimation
for (j in 1:J) {
  tmp <- data %>% filter(sub_id == j)
  obs <- tmp$x
  cond_id <- tmp$cond_id
  sig <- sd(obs)
  objective <- function(par,y=obs,kk=cond_id,s=sig) {
    intercept=par[1]; slope=par[2]
    uk <- c(-5, -1, 0)
    mk <- c()
    for (k in 1:length(uk)) {
      mk <- append(mk,get_mu(uk[k],intercept,slope))
    }
    mu <- c()
    for (n in 1:length(y)) {
      mu[n] = mk[kk[n]]
    }
    sigma <- rep(s,length(y))
    ll <- get_loglik(y,mu,sigma)
    return(ll)
  }
  print(sprintf("Subject %s",j))
  fit <- optimx(par = c("intercept" = .1, "slope" = .1),
                lower = c(0.0001, 0.0001), upper = c(1, 1),
                fn = objective, method = "bobyqa")
  param[1,j] <- fit$intercept
  param[2,j] <- fit$slope
  
  mu_cond[1,j] <- get_mu(loss = -5, intercept = fit$intercept, slope = fit$slope)
  mu_cond[2,j] <- get_mu(loss = -1, intercept = fit$intercept, slope = fit$slope)
  mu_cond[3,j] <- get_mu(loss = 0, intercept = fit$intercept, slope = fit$slope)
}

write_rds(param,"data/clean/exp2/LH_param.rds")
write_rds(mu_cond,"data/clean/exp2/LH_aims.rds")

