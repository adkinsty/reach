...
# mswu_g_yrep <- rstan::extract(readRDS("modeling/exp2/stan/rds/MSWU_gamma.rds"), 'yrep')$yrep
...
yrep_data %>%
  mutate(ratio=factor(loss,levels=c(0,-1,-5)),
         ...
         yrep=yrep*8.55,
         dist=ifelse(dist==1,"Near","Far"),
         model = factor(model,c("MEU","H","MEG","MLEG"))) %>%
  ...
  theme(axis.line=element_line(size=.25),
        ...
        legend.position = "top")
 ggsave("visuals/raw/exp2/exp2_ppc_mean_error.pdf",
