...
meg_loo <- loo(readRDS("modeling/exp2/stan/rds/MEG.rds") ,cores = 1)
mleg_loo <- loo(readRDS("modeling/exp2/stan/rds/MLEG.rds"),cores = 1)
meu_loo <- loo(readRDS("modeling/exp2/stan/rds/MEU.rds") ,cores = 1)
mswu_loo <- loo(readRDS("modeling/exp2/stan/rds/MSWU.rds") ,cores = 1)
h_loo <- loo(readRDS("modeling/exp2/stan/rds/H.rds"),cores = 1)
uh_loo <- loo(readRDS("modeling/exp2/stan/rds/UH.rds"),cores = 1)
#ulh_loo <- loo(readRDS("modeling/exp2/stan/rds/ULH.rds"),cores = 1)
#meg_vp_loo <- loo(readRDS("modeling/exp2/stan/rds/MEG_vp.rds") ,cores = 1)
...
pdat %>%
  ...
  theme(axis.line=element_line(size=.25),
        legend.pos="none")
ggsave("visuals/raw/exp2/revision/exp2_LOOIC.pdf",units="in",height=3,width=3)
